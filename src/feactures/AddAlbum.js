...
useEffect(()=>{
    if(updateDataId){
        setFormdata({
            name:updateDataId.name,description:updateDataId.description,ownerId:updateDataId.ownerId
        })
    }
},[updateDataId])
...
const handleAlbum = async (e) => {
    ...
    try{
      if(updateDataId){
         const {data} = await axios.post(`${process.env.REACT_APP_SERVER_BASE_URL}/albums/${updateDataId._id}`,formData)
     ...
     onUpdate(data.albums)
      ...
      const data = await axios.post(`${process.env.REACT_APP_SERVER_BASE_URL}/albums`,formData)
  ...
  const handleChange =(e)=>{
    ...
    return (
        ...
        <input
          ...
          required
        />
        <button className="btn btn-primary mt-2">{updateDataId?"Update":"Add"}</button>
        <button onClick={()=>onClose(false)} className="btn btn-whilte border-primary mt-2 mx-2 text-primary shadow">Cancel</button>
